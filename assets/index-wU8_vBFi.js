...
`;function ft(e){const t=document.createElement("article");t.className="backup",t.innerHTML=ht;const r=t.querySelector("section.json button.import"),i=t.querySelector("section.json button.export");return r.onclick=async()=>{const n=document.createElement("input");n.type="file",n.accept="application/json",n.onchange=async()=>{if(!n.files||n.files.length===0)return;const s=new FileReader;s.onload=async()=>await mt(s.result,e),s.onerror=()=>{alert(`Import data: read file failed: ${s.error}`)},s.readAsText(n.files[0])},n.click()},i.onclick=async()=>{const n={weekStart:f.get("weekStart"),shifts:f.get("shifts"),rhythm:f.get("rhythm"),startDate:f.get("startDate"),version:f.get("version"),indexedDB:{version:S.version,data:await S.getAll()}},s=new Date,a=(s.getMonth()+1).toString().padStart(2,"0"),o=s.getDate().toString().padStart(2,"0"),c=`shift-scheduler-backup_${s.getFullYear()}-${a}-${o}.json`;if(dt.MODE==="android")it.share({title:"Shift Scheduler Backup",text:"Backup of your Shift Scheduler data",url:(await nt.writeFile({path:c,data:JSON.stringify(n),encoding:U.UTF8,directory:H.Cache})).uri});else{const l=new Blob([JSON.stringify(n)],{type:"plain/text"});try{await navigator.share({title:"Shift Scheduler Backup",text:"Backup of your Shift Scheduler data",url:window.URL.createObjectURL(l)})}catch{const d=document.createElement("a");d.setAttribute("href",window.URL.createObjectURL(l)),d.setAttribute("download",c),d.click()}}},t}async function mt(e,t){if(typeof e!="string")return alert("Invalid data!");let r;try{r=JSON.parse(e)}catch{return alert("Invalid JSON data!")}let i;if(at(r))i=r;else if(ot(r))i=ct(r);else if(st(r))i=lt(r);else return alert("Invalid JSON data!");f.set("shifts",i.shifts),f.set("rhythm",i.rhythm),f.set("startDate",i.startDate),f.set("weekStart",i.weekStart),await S.deleteAll();for(const n of i.indexedDB.data||[])S.add(n).catch(()=>S.put(n));setTimeout(t)}const pt=D`
    <h4>IndexedDB</h4>
...
`;function gt(){const e=document.createElement("article");return e.className="db-browser",e.innerHTML=pt,e}const yt=D`
    <h4>Miscellaneous</h4>
...
`;function bt(){const e=document.createElement("article");e.className="misc",e.innerHTML=yt;const t=e.querySelector('section.week-start input[type="checkbox"]');return t.checked=f.get("weekStart")===1,t.onchange=()=>{f.set("weekStart",t.checked?1:0)},e}const vt=D`
