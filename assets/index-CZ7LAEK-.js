...
`;function De(){const e=document.createElement("article");e.className="backup",e.innerHTML=Te;const r=e.querySelector("section.json button.import"),t=e.querySelector("section.json button.export");return r.onclick=async()=>{const i=document.createElement("input");i.type="file",i.accept="application/json",i.onchange=async()=>{if(!i.files||i.files.length===0)return;const n=new FileReader;n.onload=async()=>await xe(n.result),n.onerror=()=>{alert(`Import data: read file failed: ${n.error}`)},n.readAsText(i.files[0])},i.click()},t.onclick=async()=>{const i={weekStart:u.get("weekStart"),shifts:u.get("shifts"),rhythm:u.get("rhythm"),startDate:u.get("startDate"),version:u.get("version"),indexedDB:{version:g.version,data:await g.getAll()}},n=new Blob([JSON.stringify(i)],{type:"octet/stream"}),s=document.createElement("a");s.setAttribute("href",window.URL.createObjectURL(n));const o=new Date,a=(o.getMonth()+1).toString().padStart(2,"0"),l=o.getDate().toString().padStart(2,"0"),c=`shift-scheduler-backup_${o.getFullYear()}-${a}-${l}.json`;s.setAttribute("download",c),s.click()},e}async function xe(e){if(typeof e!="string")return alert("Invalid data!");let r;try{r=JSON.parse(e)}catch{return alert("Invalid JSON data!")}let t;if(ve(r))t=r;else if(be(r))t=we(r);else if(ye(r))t=Se(r);else return alert("Invalid JSON data!");u.set("shifts",t.shifts),u.set("rhythm",t.rhythm),u.set("startDate",t.startDate),u.set("weekStart",t.weekStart),await g.deleteAll();for(const i of t.indexedDB.data||[])g.add(i).catch(()=>g.put(i))}const Me=v`
    <h4>IndexedDB</h4>
...
`;function Le(){const e=document.createElement("article");e.className="misc",e.innerHTML=Ee;const r=e.querySelector('section.week-start input[type="checkbox"]');return r.checked=u.get("weekStart")===1,r.onchange=()=>{u.set("weekStart",r.checked?1:0)},e}const $e=v`
...
`;function Be(){const e=document.createElement("article");e.className="shifts",e.innerHTML=$e;const r=()=>{const n=e.querySelector("section.shifts-table tbody"),s=e.querySelector("section.shifts-table template.table-item");n.innerHTML="",u.get("shifts").forEach(o=>{const a=s.content.cloneNode(!0).querySelector("tr.item");n.appendChild(a),a.querySelector(".name").innerText=o.name;const l=a.querySelector(".short-name");l.style.color=o.color||"inherit",l.innerText=o.visible?o.shortName:"";const c=a.querySelector("button.edit");c.onclick=async()=>{await T(o)&&(u.update("shifts",d=>d.map(p=>p.id===o.id?o:p)),r())};const m=a.querySelector("button.delete");m.onclick=async()=>{confirm(`Delete shift "${o.name}"?`)&&u.update("shifts",h=>h.filter(d=>d.id!==o.id)),r()}}),e.querySelector("section.shifts-table button.add-shift").onclick=async()=>{const o=await T(null);o&&(u.update("shifts",a=>(a.push(o),a)),r())}};r();const t=e.querySelector('section.start-date input[type="date"]');t.value=new Date(u.get("startDate")).toDateString(),t.onchange=()=>{u.set("startDate",new Date(t.value).getTime())};const i=e.querySelector("section.edit-rhythm button");return i.onclick=async()=>{u.set("rhythm",await te(u.get("rhythm"),u.get("shifts")))},e}let j="",x=null,S=[];async function Ne(){j=E(),w("Settings");const e=document.querySelector("#routerTarget");e.innerHTML="",Oe(),e.appendChild(Le()),e.appendChild(document.createElement("br")),e.appendChild(Be()),e.appendChild(document.createElement("br")),e.appendChild(De()),e.appendChild(document.createElement("br")),e.appendChild(qe())}async function Ae(){w(j),S.forEach(e=>e()),S=[]}function Oe(){const e=document.querySelector(".ui-app-bar .left .back");e.style.display="inline-flex",x=e.onclick,e.onclick=()=>q.hash.goTo(null,""),S.push(()=>{e.style.display="none",e.onclick=x})}const je=V({async onNeedRefresh(){confirm("Update verfügbar. Zum Aktualisieren bestätigen.")&&await je()}});g.open(()=>{q.hash.init(null,{"/":{title:"Shift Scheduler",template:{onMount(){he()},onDestroy(){fe()}}},settings:{title:"Shift Scheduler | Settings",template:{onMount(){Ne()},onDestroy(){Ae()}}}})});
