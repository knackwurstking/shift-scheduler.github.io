...
`;let D="",y=[];async function de(){D=x(),b("");const n=document.querySelector("#routerTarget");n.innerHTML="",n.appendChild(E.content.cloneNode(!0));const t=new ce(document.querySelector(".item-container"));fe(),me(),pe(),setTimeout(()=>{t.start()}),t.addListener("swipe",T),y.push(()=>{t.removeListener("swipe",T),t.stop()})}async function he(){b(D),y.forEach(n=>n()),y=[]}function me(){const n=new Date;Array.from(document.querySelector(".item-container").children).forEach(t=>{t.innerHTML="",t.appendChild(ne(n.getFullYear(),n.getMonth()))})}function fe(){const n=document.querySelector(".ui-app-bar .left button.date-picker"),t=document.querySelector(".ui-app-bar .right button.today"),e=document.querySelector(".ui-app-bar .right button.edit"),r=document.querySelector(".ui-app-bar .right button.printer"),i=document.querySelector(".ui-app-bar .right button.settings");n.style.display="inline-flex",t.style.display="inline-flex",e.style.display="inline-flex",r.style.display="inline-flex",i.style.display="inline-flex",n.onclick=async()=>{const s=await K(d.get("date-picker"));s&&d.set("date-picker",s.date)},t.onclick=async()=>{d.set("date-picker",new Date().getTime())},y.push(d.listen("date-picker",s=>{const o=new Date,a=new Date(s),l=a.getFullYear();l===o.getFullYear()&&a.getMonth()===o.getMonth()?t.setAttribute("disabled",""):t.removeAttribute("disabled");const c=(a.getMonth()+1).toString().padStart(2,"0");n.innerText=`${l} / ${c}`},!0)),y.push(()=>{n.style.display="none",t.style.display="none",e.style.display="none",r.style.display="none",i.style.display="none"})}function pe(){y.push(d.listen("date-picker",oe,!0),d.listen("edit-mode",ae,!0))}const ge=String.raw,L=document.createElement("template");L.innerHTML=ge`
    <article class="misc">
        <h4>Miscellaneous</h4>
...
`;let $="",w=[];async function ye(){$=x(),b("Settings");const n=document.querySelector("#routerTarget");n.innerHTML="",n.appendChild(L.content.cloneNode(!0)),be(),we(n)}async function ve(){b($),w.forEach(n=>n()),w=[]}function be(){const n=document.querySelector(".ui-app-bar .left .back");n.style.display="inline-flex",w.push(()=>{n.style.display="none"})}function we(n){Se(n),Te(n),ke(n)}function Se(n){const t=n.querySelector('article.misc section.week-start input[type="checkbox"]');t.checked=d.get("week-start")===1,t.onchange=()=>{d.set("week-start",t.checked?1:0)}}function Te(n){const t=()=>{const i=n.querySelector("article.shifts section.shifts-table tbody"),s=document.querySelector("article.shifts section.shifts-table template.table-item");i.innerHTML="",d.get("settings").shifts.forEach(o=>{const a=s.content.cloneNode(!0).querySelector("tr.item");i.appendChild(a),a.querySelector(".name").innerText=o.name;const l=a.querySelector(".short-name");l.style.color=o.color||"inherit",l.innerText=o.visible?o.shortName:"";const c=a.querySelector("button.edit");c.onclick=async()=>{await Z(o)&&(d.update("settings",u=>({...u,shifts:u.shifts.map(p=>p.id===o.id?o:p)})),t())};const f=a.querySelector("button.delete");f.onclick=async()=>{confirm(`Delete shift "${o.name}"?`)&&d.update("settings",h=>({...h,shifts:h.shifts.filter(u=>u.id!==o.id)})),t()}})};t();const e=n.querySelector('article.shifts section.start-date input[type="date"]');e.value=new Date(d.get("settings").startDate).toDateString(),e.onchange=()=>{d.update("settings",i=>(i.startDate=new Date(e.value).getTime(),i))};const r=n.querySelector("article.shifts section.edit-rhythm button");r.onclick=async()=>{const i=d.get("settings");i.rhythm=await G(i.rhythm,i.shifts),d.set("settings",i)}}function ke(n){const t=n.querySelector("article.backup section.json button.import"),e=n.querySelector("article.backup section.json button.export");t.onclick=async()=>{const r=document.createElement("input");r.type="file",r.accept="application/json",r.onchange=async()=>{if(!r.files||r.files.length===0)return;const i=new FileReader;i.onload=()=>{},i.onerror=()=>{alert(`Import data: read file failed: ${i.error}`)},i.readAsText(r.files[0])},r.click()},e.onclick=async()=>{const r=new Blob([JSON.stringify({weekStart:d.get("week-start"),settings:d.get("settings"),indexedDB:{version:v.version,data:await v.getAll()}})],{type:"octet/stream"}),i=document.createElement("a");i.setAttribute("href",window.URL.createObjectURL(r));const s=new Date,o=(s.getMonth()+1).toString().padStart(2,"0"),a=s.getDate().toString().padStart(2,"0"),l=`shift-scheduler-backup_${s.getFullYear()}-${o}-${a}.json`;i.setAttribute("download",l),i.click()}}const xe=_({async onNeedRefresh(){confirm("Update verfügbar. Zum Aktualisieren bestätigen.")&&await xe()}});v.open(()=>{O.hash.init(null,{"/":{title:"Shift Scheduler",template:{onMount(){de()},onDestroy(){he()}}},settings:{title:"Shift Scheduler | Settings",template:{onMount(){ye()},onDestroy(){ve()}}}})});
