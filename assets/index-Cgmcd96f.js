...
`;const Ie=[0,1,2,3,4,5,6],Be=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];function je(e,t){const r=ie.content.cloneNode(!0).querySelector(".item-content");for(let i=0;i<6;i++){const n=document.createElement("div");r.appendChild(n),n.className="days ui-flex-grid-item ui-flex-grid-row",n.style.setProperty("--gap","0.05rem");for(let s=0;s<7;s++)n.append(Ue(0))}return r.onclick=Ne,setTimeout(()=>{se(r,e,t)}),r}async function se(e,t,r){const i=new Date(t,r),n=Array.from(e.querySelectorAll(".item-content > .days > .day"));let s=await Ee(n.length,i.getFullYear(),i.getMonth());const a=f.get("shifts");s.forEach(async(o,c)=>{const l=await S.get(o.year,o.month,o.date);if(l!==null){o.note=l.note;const d=a.find(h=>{var u;return h.id===((u=l.shift)==null?void 0:u.id)});d?o.shift=d:o.shift=l.shift||o.shift}oe(n[c],o),r!==o.month?n[c].setAttribute("disabled",""):n[c].removeAttribute("disabled"),o.year!==i.getFullYear()||o.month!==i.getMonth()?n[c].classList.add("inactive"):n[c].classList.remove("inactive")}),He([...e.querySelectorAll(".item-content > .week-days > .week-day")],n)}async function Ne(e){const t=e.target.closest(".day");if(!t)return;const r=parseInt(t.getAttribute("data-year"),10),i=parseInt(t.getAttribute("data-month"),10),n=parseInt(t.getAttribute("data-date"),10),s=parseInt(t.getAttribute("data-shift-id")||"0",10),a=f.get("editMode");let o;if(!a.open)o=await Pe(new Date(r,i,n,6,0,0),s,await S.get(r,i,n));else{const d=await S.get(r,i,n);o={shiftID:a.active?a.active:0,note:(d==null?void 0:d.note)||""}}if(!o)return;const c=ne(new Date(r,i,n)),l={year:r,month:i,date:n,shift:f.get("shifts").find(d=>d.id===o.shiftID)||null,note:o.note||""};l.shift&&c&&c.id===l.shift.id&&(l.shift=null),l.note||l.shift?(console.warn("Add data to the database",l),S.put(l).catch(()=>S.add(l).catch(d=>alert(`Update database failed: ${d}`)))):(console.warn("Remove data from the database",{year:r,month:i,day:n}),S.delete(r,i,n)),l.shift||(l.shift=c),oe(t,l)}function He(e,t){const r=f.get("weekStart");let i=Ie;r>0&&(i=[...i.slice(r),...i.slice(0,r)]);const n=i.findIndex(a=>a===6),s=i.findIndex(a=>a===0);e.forEach((a,o)=>{a.innerHTML=`${Be[i[o%7]]}`}),[...e,...t].forEach((a,o)=>{a.classList.remove("saturday"),a.classList.remove("sunday"),o%i.length===n&&a.classList.add("saturday"),o%i.length===s&&a.classList.add("sunday")})}function Ue(e,t){const r=document.createElement("div");return r.className="day ui-flex-grid-item",r.innerHTML=D`
        <div class="date">${e}</div>
        <div class="shift">${""}</div>
    `,r}function oe(e,t){((n,s,a)=>{const o=new Date;return o.getFullYear()===n&&o.getMonth()===s&&o.getDate()===a})(t.year,t.month,t.date)?e.classList.add("today"):e.classList.remove("today"),t.note?e.classList.add("note"):e.classList.remove("note"),e.querySelector(".date").innerHTML=`${t.date}`;const i=e.querySelector(".shift");t.shift?(i.style.setProperty("--shift-color",t.shift.visible?t.shift.color||"inherit":"transparent"),i.innerHTML=t.shift.shortName||""):(i.style.removeProperty("--shift-color"),i.innerHTML=""),e.setAttribute("data-year",t.year.toString()),e.setAttribute("data-month",t.month.toString()),e.setAttribute("data-date",t.date.toString()),t.shift&&e.setAttribute("data-shift-id",t.shift.id.toString())}async function Fe(e){const t=new Date(e);t.setMonth(t.getMonth()-1),Array.from(document.querySelector(".item-container").children).forEach(async r=>{se(r.querySelector(".item-content"),t.getFullYear(),t.getMonth()),t.setMonth(t.getMonth()+1)})}async function _e(e){const r=document.querySelector("#routerTarget").querySelector(".item-container"),i=document.querySelector(".edit-mode");!r||!i||(i.innerHTML="",e.open?(r.setAttribute("edit-mode",""),i.setAttribute("open",""),Xe(i,e.active)):(r.removeAttribute("edit-mode"),i.removeAttribute("open")))}async function Y(e){switch(e){case"left":f.update("datePicker",t=>{const r=new Date(t);return r.setMonth(r.getMonth()+1),r.getTime()});break;case"right":f.update("datePicker",t=>{const r=new Date(t);return r.setMonth(r.getMonth()-1),r.getTime()});break}}function Xe(e,t){const r=document.querySelector('template[name="shift-card"]');(f.get("shifts")||[]).forEach(i=>{const n=r.content.cloneNode(!0).querySelector(".shift-card");t===i.id?(n.setAttribute("active",""),n.scrollIntoView()):n.removeAttribute("active"),n.querySelector(".name").innerText=i.name;const s=n.querySelector(".short-name");s.style.color=i.color||"inherit",s.innerText=i.visible?i.shortName:"",n.onclick=()=>{n.hasAttribute("active")?n.removeAttribute("active"):n.setAttribute("active",""),f.update("editMode",a=>(a.active=n.hasAttribute("active")?i.id:0,a))},e.appendChild(n)})}const ae=document.createElement("template");ae.innerHTML=D`
    <style>
        html,
        body {
...
`;function yt(){const e=document.createElement("article");e.className="shifts",e.innerHTML=gt;const t=()=>{const s=e.querySelector("section.shifts-table tbody"),a=e.querySelector('section.shifts-table template[name="table-item"]');s.innerHTML="",f.get("shifts").forEach(o=>{const c=a.content.cloneNode(!0).querySelector("tr.item");s.appendChild(c),c.querySelector(".name").innerText=o.name;const l=c.querySelector(".short-name");l.style.color=o.color||"inherit",l.innerText=o.visible?o.shortName:"";const d=c.querySelector("button.edit");d.onclick=async()=>{const u=await V(o);u&&(f.update("shifts",p=>p.map(m=>m.id===u.id?u:m)),t())};const h=c.querySelector("button.delete");h.onclick=async()=>{confirm(`Delete shift "${o.name}"?`)&&f.update("shifts",u=>u.filter(p=>p.id!==o.id)),t()}}),e.querySelector("section.shifts-table button.add-shift").onclick=async()=>{const o=await V(null);o&&(f.update("shifts",c=>(c.push(o),c)),t())}};t();const r=e.querySelector('section.start-date input[type="date"]');if(f.get("startDate")){const s=new Date,a=(s.getMonth()+1).toString().padStart(2,"0"),o=s.getDate().toString().padStart(2,"0");r.value=`${s.getFullYear()}-${a}-${o}`,r.onchange=()=>{f.set("startDate",new Date(r.value).getTime())}}const n=e.querySelector("section.edit-rhythm button");return n.onclick=async()=>{f.set("rhythm",await $e(f.get("rhythm"),f.get("shifts")))},e}let ue="",Z=null,H=[];async function de(){ue=te(),P("Settings");const e=document.querySelector("#routerTarget");e.innerHTML="",bt(),e.appendChild(pt()),e.appendChild(document.createElement("br")),e.appendChild(yt()),e.appendChild(document.createElement("br")),e.appendChild(ut(vt)),e.appendChild(document.createElement("br")),e.appendChild(ft())}async function he(){P(ue),H.forEach(e=>e()),H=[]}function bt(){const e=document.querySelector(".ui-app-bar .left .back");e.style.display="inline-flex",Z=e.onclick,e.onclick=()=>ee.hash.goTo(null,""),H.push(()=>{e.style.display="none",e.onclick=Z})}async function vt(){await he(),await de()}console.debug({PWA:!0,MODE:"github"});{console.debug("PWA updater registered");const e=Te({async onNeedRefresh(){confirm("Update verfügbar. Zum Aktualisieren bestätigen.")&&await e()}})}S.open(()=>{ee.hash.init(null,{"/":{title:"Shift Scheduler",template:{onMount(){We()},onDestroy(){Re()}}},settings:{title:"Shift Scheduler | Settings",template:{onMount(){de()},onDestroy(){he()}}}})});export{G as E,le as W,tt as b};
