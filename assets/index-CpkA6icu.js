...
`;function Be(){const e=document.createElement("article");e.className="shifts",e.innerHTML=$e;const r=()=>{const n=e.querySelector("section.shifts-table tbody"),s=e.querySelector("section.shifts-table template.table-item");n.innerHTML="",c.get("shifts").forEach(o=>{const a=s.content.cloneNode(!0).querySelector("tr.item");n.appendChild(a),a.querySelector(".name").innerText=o.name;const l=a.querySelector(".short-name");l.style.color=o.color||"inherit",l.innerText=o.visible?o.shortName:"";const u=a.querySelector("button.edit");u.onclick=async()=>{await ee(o)&&(c.update("shifts",d=>d.map(p=>p.id===o.id?o:p)),r())};const m=a.querySelector("button.delete");m.onclick=async()=>{confirm(`Delete shift "${o.name}"?`)&&c.update("shifts",h=>h.filter(d=>d.id!==o.id)),r()}})};r();const t=e.querySelector('section.start-date input[type="date"]');t.value=new Date(c.get("startDate")).toDateString(),t.onchange=()=>{c.set("startDate",new Date(t.value).getTime())};const i=e.querySelector("section.edit-rhythm button");return i.onclick=async()=>{c.set("rhythm",await te(c.get("rhythm"),c.get("shifts")))},e}let O="",x=null,S=[];async function Ne(){O=q(),w("Settings");const e=document.querySelector("#routerTarget");e.innerHTML="",Oe(),e.appendChild(Le()),e.appendChild(document.createElement("br")),e.appendChild(Be()),e.appendChild(document.createElement("br")),e.appendChild(xe()),e.appendChild(document.createElement("br")),e.appendChild(qe())}async function Ae(){w(O),S.forEach(e=>e()),S=[]}function Oe(){const e=document.querySelector(".ui-app-bar .left .back");e.style.display="inline-flex",x=e.onclick,e.onclick=()=>M.hash.goTo(null,""),S.push(()=>{e.style.display="none",e.onclick=x})}const je=Y({async onNeedRefresh(){confirm("Update verfügbar. Zum Aktualisieren bestätigen.")&&await je()}});g.open(()=>{M.hash.init(null,{"/":{title:"Shift Scheduler",template:{onMount(){he()},onDestroy(){fe()}}},settings:{title:"Shift Scheduler | Settings",template:{onMount(){Ne()},onDestroy(){Ae()}}}})});
