...
`;function Be(e){const r=document.createElement("article");r.className="backup",r.innerHTML=Ne;const t=r.querySelector("section.json button.import"),i=r.querySelector("section.json button.export");return t.onclick=async()=>{const n=document.createElement("input");n.type="file",n.accept="application/json",n.onchange=async()=>{if(!n.files||n.files.length===0)return;const o=new FileReader;o.onload=async()=>await Ie(o.result,e),o.onerror=()=>{alert(`Import data: read file failed: ${o.error}`)},o.readAsText(n.files[0])},n.click()},i.onclick=async()=>{const n={weekStart:u.get("weekStart"),shifts:u.get("shifts"),rhythm:u.get("rhythm"),startDate:u.get("startDate"),version:u.get("version"),indexedDB:{version:y.version,data:await y.getAll()}},o=new Blob([JSON.stringify(n)],{type:"plain/text"}),a=new Date,s=(a.getMonth()+1).toString().padStart(2,"0"),l=a.getDate().toString().padStart(2,"0"),c=`shift-scheduler-backup_${a.getFullYear()}-${s}-${l}.json`;try{await navigator.share({title:"Shift Scheduler Backup",text:"Backup of your Shift Scheduler data",files:[new File([o],c,{type:"plain/text"})]})}catch{const d=document.createElement("a");d.setAttribute("href",window.URL.createObjectURL(o)),d.setAttribute("download",c),d.click()}},r}async function Ie(e,r){if(typeof e!="string")return alert("Invalid data!");let t;try{t=JSON.parse(e)}catch{return alert("Invalid JSON data!")}let i;if(Ee(t))i=t;else if(Me(t))i=Le(t);else if(qe(t))i=$e(t);else return alert("Invalid JSON data!");u.set("shifts",i.shifts),u.set("rhythm",i.rhythm),u.set("startDate",i.startDate),u.set("weekStart",i.weekStart),await y.deleteAll();for(const n of i.indexedDB.data||[])y.add(n).catch(()=>y.put(n));setTimeout(r)}const Ce=v`
    <h4>IndexedDB</h4>
